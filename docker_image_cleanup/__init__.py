...
def clean_repo(client, image_repo, num_recent, min_age_days, dry_run, now):
    ...
    for img_id, remove_tags in remove_by_id.items():
        ...
        if set(remove_tags) == set(all_tags):
            in_use = len(client.containers.list(all=True, filters={'ancestor': img_id})) > 0
            if in_use:
                log.warning("skipped image in use", image_id=img_id, tags=remove_tags)
                continue
            size = img.attrs.get('Size', 0)
            if dry_run:
                log.info("would remove image", image_id=img_id, tags=remove_tags, size=human_size(size))
            else:
                for tag in remove_tags:
                    client.images.remove(tag, force=False)
                log.info("removed image", image_id=img_id, tags=remove_tags, size=human_size(size))
            total_saved += size
        else:
            if dry_run:
...
@click.option('--num-recent', default=5, help='Number of recent tags to keep.', show_default=True)
@click.option('--min-age-days', default=30, help='Minimum age in days to keep tags.', show_default=True)
...
def main(image_repos, num_recent, min_age_days, dry_run):
    ...
    for image_repo in image_repos:
        ...
        grand_total += clean_repo(client, image_repo, num_recent, min_age_days, dry_run, now)

    if len(image_repos) > 1:
        msg = "grand total space that would be saved" if dry_run else "grand total space saved"
